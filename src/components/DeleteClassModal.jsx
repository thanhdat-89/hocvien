...
const DeleteClassModal = ({ classItem, onDelete, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await onDelete(classItem.id, password);
            onClose();
        } catch (err) {
        ...
        <div className="modal-overlay">
            <div className="modal-content card" style={{ maxWidth: '400px', width: '90%', padding: '1.5rem', border: '1px solid var(--danger)' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: '1.5rem' }}>
                    ...
                    <h2 style={{ color: 'var(--danger)', marginBottom: '0.5rem', fontSize: '1.25rem' }}>Xác nhận xóa lớp học</h2>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: '1.5' }}>
                        Bạn đang chuẩn bị xóa lớp học <strong>{classItem.name}</strong>. Hành động này sẽ xóa vĩnh viễn lớp học khỏi hệ thống.
                    </p>
                </div>
